...
#  на том же внешнем TCP/443. Опционально поднимает Remnanode. Автоматически
#  устраняет конфликт Caddy/rw-core за 443 и проверяет стрим-сайт.
#
...
verify_site_port() {
  ...
  die "Стрим-сайт не прошёл проверку на порту ${port}: HTTP ${code:-нет ответа}."
}
...
${B}④ Готовые inbound для Config Profile:${N}
  ...
  Если профиль уже содержит REALITY на 443, установщик сам переносит Caddy на ${CADDY_LOCAL_PORT} и перезапускает ноду.
...
auto_finish_after_node() {
  local i
  log "Проверяю, какой профиль назначен ноде..."
  for i in $(seq 1 30); do
    if ss -lntp 2>/dev/null | grep -E ':443[[:space:]]' | grep -q 'rw-core'; then
      reality_enable_impl
      return 0
    fi
    if node_has_443_conflict; then
      warn "В профиле уже есть REALITY на :443; автоматически переношу Caddy на 127.0.0.1:${CADDY_LOCAL_PORT}."
      reality_enable_impl
      return 0
    fi
    if ss -lntp 2>/dev/null | grep -q "127.0.0.1:${BACKEND_PORT}"; then
      ok "Профиль XHTTP применён; Caddy пока остаётся публичным на :443."
      verify_site_port 443
      return 0
    fi
    sleep 2
  done
  warn "Профиль ещё не применён. Caddy и сайт работают на :443; готовые inbound JSON уже созданы."
  verify_site_port 443
}
...
cmd_reality_prepare() {
  ...
  say "  После сохранения профиля выполни: ${C}${SCRIPT_INSTALL_PATH} reality-enable${N}"
  line
}

reality_enable_impl() {
  resolve_existing
  if ss -lntp 2>/dev/null | grep -E ':443[[:space:]]' | grep -q 'rw-core' && \
     ss -lntp 2>/dev/null | grep -q "127.0.0.1:${CADDY_LOCAL_PORT}"; then
    fix_site_permissions
    verify_site_port 443
