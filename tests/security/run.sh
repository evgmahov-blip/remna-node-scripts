...
grep -q '^SCANNER_URL=https://lists.blocklist.de/lists/all.txtbegin docker
SRC=$WORK/src
write_pair "$SRC" 8
nets 15 20 > "$WORK/scanners.txt"
export REMNA_SECURITY_SOURCE_SCANNERS=file://$WORK/scanners.txt
...
bash "$ROOT/tests/security/v2/run.sh"
ok v2
 "$REMNA_SECURITY_BASE/settings.conf" || fail "default scanner feed missing"
grep -q '^ENABLE_GEOIP=0begin docker
SRC=$WORK/src
write_pair "$SRC" 8
nets 15 20 > "$WORK/scanners.txt"
export REMNA_SECURITY_SOURCE_SCANNERS=file://$WORK/scanners.txt
...
bash "$ROOT/tests/security/v2/run.sh"
ok v2
 "$REMNA_SECURITY_BASE/settings.conf" || fail "geoip must stay off by default"
grep -q '^LOG_DROPS=0begin docker
SRC=$WORK/src
write_pair "$SRC" 8
nets 15 20 > "$WORK/scanners.txt"
export REMNA_SECURITY_SOURCE_SCANNERS=file://$WORK/scanners.txt
...
bash "$ROOT/tests/security/v2/run.sh"
ok v2
 "$REMNA_SECURITY_BASE/settings.conf" || fail "drop logging must stay off by default"
st=$(bash "$CLI" status --json)
