...
PREFERRED_PERCENT="${SELFSTEAL_PREFERRED_PERCENT:-80}"
PREFERRED_SITES=(stream template:rybaliti-2.0 template:worldzoo-stream-template)
...
deploy_site_value(){
  ...
  case "$site" in
    ...
    radio) deploy_radio ;;
    template:*) deploy_template "${site#template:}" ;;
    *) fail "Неизвестный SelfSteal site value: $site"; return 1 ;;
  esac
}

deploy_random_preferred(){
  local current="" pick="" i idx
  local -a pool=()
  [[ "$PREFERRED_PERCENT" =~ ^[0-9]+$ ]] && (( PREFERRED_PERCENT >= 0 && PREFERRED_PERCENT <= 100 )) \
    || { fail 'SELFSTEAL_PREFERRED_PERCENT должен быть 0..100'; return 1; }
  [[ -r "$STATE_FILE" ]] && current="$(tr -d '\r\n' < "$STATE_FILE")"

  if (( RANDOM % 100 < PREFERRED_PERCENT )); then
    pool=("${PREFERRED_SITES[@]}")
  else
    for i in "${TEMPLATES[@]}"; do
      [[ "$i" == "rybaliti-2.0" || "$i" == "worldzoo-stream-template" ]] && continue
      pool+=("template:$i")
    done
  fi
  ...
  for _ in 1 2 3 4 5 6; do
    idx=$((RANDOM % ${#pool[@]}))
    pick="${pool[$idx]}"
    [[ "${#pool[@]}" -le 1 || "$pick" != "$current" ]] && break
  done

  log "[INFO] SelfSteal AUTO: favorites=${PREFERRED_PERCENT}% | выбран: $pick"
  deploy_site_value "$pick"
}

deploy_random_favorite(){
  local current="" pick="" idx
  [[ -r "$STATE_FILE" ]] && current="$(tr -d '\r\n' < "$STATE_FILE")"
  for _ in 1 2 3 4 5 6; do
    idx=$((RANDOM % ${#PREFERRED_SITES[@]}))
    pick="${PREFERRED_SITES[$idx]}"
    [[ "$pick" != "$current" || "${#PREFERRED_SITES[@]}" -le 1 ]] && break
  done
  deploy_site_value "$pick"
}
...
ensure_site(){
  ...
  case "$selected" in
    stream) deploy_stream ;;
    radio) deploy_radio ;;
    random) deploy_random_preferred ;;
    template:*) template="${selected#template:}"; deploy_template "$template" ;;
    *) log "[WARN] Неизвестное сохранённое значение '$selected'; ставлю AUTO RANDOM"; deploy_random_preferred ;;
  esac
}
...
choose_site(){
  ...
  echo ' 11) AUTO RANDOM — 80% любимые / 20% остальные'
  echo ' 12) ⭐ СЛУЧАЙНЫЙ из любимых: STREAM / rybaliti / worldzoo'
  echo ' 13) СЛУЧАЙНЫЙ из старых профессиональных шаблонов'
  echo '  0) Назад'
  ...
  case "${choice:-1}" in
    ...
    11) deploy_random_preferred ;;
    12) deploy_random_favorite ;;
    13) deploy_random_template ;;
    0) return 0 ;;
...
main(){
  ...
  case "${1:-ensure}" in
    ...
    radio) deploy_radio ;;
    random) deploy_random_preferred ;;
    favorite) deploy_random_favorite ;;
    template) [[ -n "${2:-}" ]] || { fail 'Укажи имя шаблона'; return 1; }; deploy_template "$2" ;;
    status) show_status ;;
    *) fail 'Допустимо: ensure | choose | stream | radio | random | favorite | template NAME | status'; return 1 ;;
  esac
}
