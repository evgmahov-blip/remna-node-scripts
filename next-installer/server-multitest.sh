...
test_ipquality(){
  ...
  # Upstream IPQuality has a known shell-status quirk: on IPv4-only hosts its final
  # IPv6 conditional may leave the script with rc=1 even after a successful IPv4 run.
  # Use its documented JSON output file instead of stdout and judge success by JSON.
  set +e
  ...
  if [[ -s "$json" ]] && jq -e . "$json" >/dev/null 2>&1; then
    ...
    if [[ -s "$stderr" ]]; then
      ...
      tail -n 8 "$stderr" | sed -E 's/\x1B\[[0-9;?]*[ -\/]*[@-~]//g' >&2 || true
    fi
    rm -f "$script" "$json" "$stderr"
    return 1
  fi
